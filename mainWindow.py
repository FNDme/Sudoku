...
# crear canvas y redondear las esquinas
canvas = Canvas(base, width=440, height=440, bg="#0E7344", highlightthickness=0)
...
# Función para formatear el tablero para
def boardFormat(tab):
    parameter = str(tab)
...
def nextSol():
    ...
    if whichSol < nSolutions:
        ...
        solList.pop(0)
        whichSol += 1
        nSolLabel.config(text="Solución " + str(whichSol) + " de " + str(nSolutions))

# Funcion Resolución de tablero tradicional
def solveSudoku(sudokuType):
    ...
    for i in range(9):
        for j in range(9):
            if entrys[i][j].get() in ["1", "2", "3", "4", "5", "6", "7", "8", "9"]:
                ...
                messagebox.showinfo("Error", "El valor de la casilla " + str(i+1) + "x" + str(j+1) + " debe ser un número entre 1 y 9")
                return
    ...
    try:
        ...
        messagebox.showinfo("Error", "El número de soluciones debe ser un número entero")
        return
...
for i in range(9):
    for j in range(9):
        ...
        entrys[i][j].config(bg="#F2B705", font=("Arial", 20), fg="#025939", highlightthickness=0.5, highlightbackground="#025939", border=0, readonlybackground="#F29F05")
        if (i in [0,1,2] and j in [0,1,2,6,7,8]) or (i in [3,4,5] and j in [3,4,5]) or (i in [6,7,8] and j in [0,1,2,6,7,8]):
            entrys[i][j].config(bg="#F29F05", readonlybackground="#F2B705")

title = Label(base, text="Sudoku solver", font=("verdana", 23, BOLD), bg="#025939", fg="#F2B705")
title.place(x=20, y=60)

solveClassicSudokuButton = Button(base, text="Sudoku Classic", command=partial(solveSudoku, "tradicional"), bg="#0E7344", fg="#F2B705", font=("Arial", 12, BOLD), border=0, highlightthickness=0.5, highlightbackground="#025939")
solveClassicSudokuButton.place(x=70, y=160, width=160, height=40)

solveDiagonalSudokuButton = Button(base, text="Sudoku Diagonal", command=partial(solveSudoku, "diagonal"), bg="#0E7344", fg="#F2B705", font=("Arial", 12, BOLD), border=0, highlightthickness=0.5, highlightbackground="#025939")
solveDiagonalSudokuButton.place(x=70, y=220, width=160, height=40)

solveWinSudokuButton = Button(base, text="Sudoku Win", command=partial(solveSudoku, "win"), bg="#0E7344", fg="#F2B705", font=("Arial", 12, BOLD), border=0, highlightthickness=0.5, highlightbackground="#025939")
solveWinSudokuButton.place(x=70, y=280, width=160, height=40)

resetButton = Button(base, text="Reset", command=resetBoard, bg="#0E7344", fg="#F2B705", font=("Arial", 12, BOLD), border=0, highlightthickness=0.5, highlightbackground="#025939", state="disabled")
resetButton.place(x=70, y=370, width=160, height=40)

nextButton = Button(base, text="Next", command=nextSol, bg="#0E7344", fg="#F2B705", font=("Arial", 12, BOLD), border=0, highlightthickness=0.5, highlightbackground="#025939", state="disabled")
nextButton.place(x=70, y=430, width=160, height=40)

#label
label = Label(base, text="Number of possible solutions", font=("Arial", 10), bg="#025939", fg="#F2B705")
label.place(x=30, y=497)

nSol = Entry(base, justify="center")
nSol.place(x=220, y=500, width=40, height=20)
nSol.insert(0, "1")
nSol.config(bg="#F2B705", font=("Arial", 12), fg="#025939", highlightthickness=0.5, highlightbackground="#025939", border=0)

nSolLabel = Label(base, text="", font=("Arial", 10), bg="#025939", fg="#F2B705")
